...
namespace BulletSharp
{
	public static class GeometryUtil
	{
        ...
        public static List<Vector3> GetVerticesFromPlaneEquations(ICollection<Vector3> planeEquations)
        {
            int numPlanes = planeEquations.Count;
            Vector4[] planes = new Vector4[numPlanes];
            int i = 0;
            foreach (Vector3 plane in planeEquations)
            {
                planes[i] = new Vector4(plane.X, plane.Y, plane.Z, 0f);
                i++;
            }
            return GetVerticesFromPlaneEquations(planes);
        }
